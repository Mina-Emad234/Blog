...
if ($sess->has('session-id') and $_SESSION['session-id'] != NULL) {//if session_id exists
    ...
    if (!is_null($user_rand_id) || !empty($user_rand_id)) {//check if rand_id exists
        $user_model = new UserModel();
        $user = $user_model->getUserDetails($user_rand_id);//get all user data
    }
}
?>
<!DOCTYPE html>
        ...
        <div class="col-4 d-flex justify-content-end align-items-center">
          ...
          <div class="dropdown ml-2">
            <button class="btn btn-outline-secondary dropdown-toggle btn-sm" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                <img src="<?= '/uploads/' . $user['image']?>" class="rounded-circle" width="20" height="20" title="<?= $user['user_name']?>"> <?= $user['user_fname']. " " .  $user['user_lname']?>
            </button>
            <div class="dropdown-menu dropdown-menu-right dropdown-menu-lg-left" aria-labelledby="dropdownMenuButton">
              ...
              <a class="dropdown-item mt-1" href="<?=base_url()?>/users/categories"><i class="fa fa-object-group" aria-hidden="true"></i>
                Create a Category</a>
                <?php if ($sess->has('admin')): ?>
                <a class="dropdown-item mt-1" href="<?=base_url()?>/admin/posts/all"><i class="fa fa-th" aria-hidden="true"></i>
                All Posts</a>
                <a class="dropdown-item mt-1" href="<?=base_url()?>/admin/comments/all"><i class="fa fa-comment" aria-hidden="true"></i>
                All Comments</a>
                <?php endif; ?>
              <div class="dropdown-divider"></div>
              <a class="dropdown-item" href="/users/logout"><i class="fa fa-sign-out" aria-hidden="true"></i>
                Logout</a>
            </div>
          </div>
          <?php endif; ?>
